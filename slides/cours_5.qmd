---
subtitle: "Développer une librairie R"
---
...
- Il est possible d'écrire les documents .Rd à la main. Mais il est recommandé d'utiliser `roxygen2` pour les générer automatiquement !
...
`devtools::document()` crée les fichiers dans `man/` à partir des balises `Roxygen` détectées dans `R/`
...
- Les usages d'un package vont souvent au-delà d'un appel à une seule fonction
- Pour documenter des usages plus complexes, on utilise les *vignettes*.
...
`usethis::use_vignette()` crée une nouvelle vignette à compléter dans `vignettes/`
...
`usethis::use_package("dplyr", type = "Imports")` ajoute une dépendance de type _import_ pour `dplyr` dans `DESCRIPTION`
...
- Le fichier `README.md` sera la porte d'entrée de votre répertoire sur GitHub.
- Il doit rapidement décrire comment installer et utiliser votre package.
- Il peut être illustré avec un logo (_hex sticker_)

::: {.callout-note}

## usethis et devtools

`usethis::use_readme_rmd()` pour créer le fichier `.rmd` de source.

`devtools::build_readme()` pour créer le `.md` (équivaut à faire un `render`)

:::

## Metadata : NEWS, LICENCE et COC

### NEWS

- Liste les nouvelles _features_ pour chaque release.
- Chaque entrée devrait correspondre à une version différente du package.

### LICENCE / CODE_OF_CONDUCT

- Spécifie les réglementations de partage, réutilisation de code.
- Chaque type de licence équivaut à une [permissivité particulière](https://usethis.r-lib.org/reference/licenses.html).
- Le CoC spécifie la réglementation de participation au dévelopement du pakage.

::: {.callout-note}

## usethis

`usethis::use_[mit/ccby/proprietary]_license`, `usethis::use_news_md()` et `usethis::use_code_of_conduct()` pour créer ces fichiers.

`usethis::use_version()` pour incrémenter la version de votre package.
...
[`{pkgdown}`](https://pkgdown.r-lib.org/) est un package permettant de transformer son code en une page de documentation HTML complète.

Elle peut être hébergée sur GitHub via les Pages et automatiquement mise à jour via les Actions.

::: {.callout-note}

## usethis / pkgdown

`usethis::use_pkgdown()` pour configurer votre répertoire et les `.gitignore`.

`pkgdown::build_site()` pour créer la documentation dans un dossier `docs/` local.

`sethis::use_pkgdown_github_pages()` pour configurer un déploiement automatique depuis GitHub.
