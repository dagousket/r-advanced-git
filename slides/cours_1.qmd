---
subtitle: "IDE RStudio, rappels de R"
---
...
```{r}
c(1, 2.3, 4.5, -6.6)  # double
c(1L, 2L, 22L)  # entier
1:5  # entier sequentiel
c("ceci", "est", "un vecteur")  # character
```
...
```{r}
#| eval: true
a  <- list(premier = c(3,6), deuxieme = c("beurre", "sucre"))
a
```
...
[Illustration by Hadley Wickham in [Advanced R](https://adv-r.hadley.nz/subsetting.html#section)]{.aside}
...
# if / else if / else
if (a > 40) {
  print("c'est plus que 40")
} else if (a < 20) {
  print("c'est moins que 20")
} else {
  print("c'est autre chose")
}
# switch
switch(a > 40, "c'est plus que 40", "c'est 40 ou moins")
# ifelse vectorisé
ifelse(test = c(TRUE, TRUE, FALSE), yes = "A", no = "B")
```
...
- Le code est à destination de l'ordinateur, mais aussi par vos collègues humains (et votre _vous_ futur).
- Il faut respecter des conventions
...
On peut également utiliser des styler externes comme [air](https://posit-dev.github.io/air/).
