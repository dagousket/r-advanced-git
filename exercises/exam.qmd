...
- Assurez-vous d'avoir eu la **confirmation** de votre dépôt avant de quitter la salle.
...
\* Préciser la source du jeu de donnée dans un fichier `data-raw/DATASET.R` comme suit :

```{bash}
#| eval: false
## code to prepare `DATASET` dataset goes here

url_jeu_de_donnees <- 'https://data.loire-atlantique.fr/explore/dataset/323266205_alerte-pollens-nantes%40nantesmetropole/information/'
...
```{=typst}
- [ ] Le dossier rendu est un répertoire git initié pendant l'examen
- [ ] L'historique des commits contient au moins un `merge` via une _Pull Request_ GitHub
...
- [ ] La phrase suivante est complétée et ajoutée dans le README :
    - _Le premier commit de ce répertoire a été effectué le xx/xx/xxxx à xxhxx_
...
- [ ] Une des fonctions du package réalise un calcul groupé sur le jeu de données (_eg_ `mean`, `max`, `median` par groupe)
- [ ] Une des fonctions du package réalise un graphique `ggplot2` à partir du calcul groupé précédent
- [ ] Une des fonctions du package comporte au minimum 2 paramètres d'entrée
```
